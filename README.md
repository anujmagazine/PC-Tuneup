...
- 🌡️ **Fan noise diagnosis** — identifies which specific app is spinning your fan right now, with per-app fix advice for 20+ known apps (Chrome, Teams, Zoom, OneDrive, Windows Defender, Steam, and more); also detects thermal lag when CPU is calm but the laptop is still cooling down
- 🧠 **Smart memory leak detection** — monitors RAM growth every 60 seconds, skips Windows system processes, and gives per-app tailored advice
- 🔒 **100% local** — nothing is sent anywhere, ever

### All 29 Problems It Detects & Fixes
...
| # | Problem | How the App Helps | Fix Method |
|---|---|---|---|
| 8 | **High CPU usage** | Detects when CPU is overloaded (>80%) and names the specific culprit app | Go to Running Apps tab → fan noise section |
| 9 | **Fan noise / CPU hog** | Background thread samples CPU every 3 seconds; ranks all apps by CPU%; gives specific fix steps per app; detects thermal lag | Per-app fix advice; Force Close button |
| 10 | **Thermal throttling** | Reads CPU temperature via WMI, detects when CPU slows itself due to heat | Shows temp, gives cooling tips |
| 11 | **High CPU temperature** | Warns when CPU is getting warm (>75°C) before throttling starts | Preventive tips |
| 12 | **Slow power plan** | Detects if Windows is in Balanced/Power Saver mode | One-click switch to High Performance |
| 13 | **Visual effects wasting CPU** | Windows animations, transparency, and effects use CPU/GPU | One-click disable (sets Best Performance) |
| 14 | **CPU frequency throttling** | Compares current CPU speed to max speed | Detects if CPU is running below capacity |

#### Memory Issues

| # | Problem | How the App Helps | Fix Method |
|---|---|---|---|
| 15 | **High RAM usage** | Alerts when RAM exceeds 70% or 85% | One-click memory free + identifies heavy apps |
| 16 | **Memory-hungry processes** | Finds apps using 500+ MB of RAM each | Lists top 30 processes with "End" button |
| 17 | **Memory leaks** | Background monitor tracks process RAM every 60 seconds; excludes system processes; gives per-app tailored fix advice | Shows growth rate, advises app-specific fix |
| 18 | **Excessive paging/swap** | Detects when Windows is using the slow hard drive as overflow RAM (>50% swap) | One-click memory free + app cleanup guidance |
| 19 | **Standby memory buildup** | Cached memory that isn't released back to apps that need it | One-click working set trim for all processes |

#### Startup & Background Issues

| # | Problem | How the App Helps | Fix Method |
|---|---|---|---|
| 20 | **Too many startup programs** | Counts programs from Registry (HKCU & HKLM) and Startup folder | Lists all with source; opens Task Manager Startup tab |
| 21 | **Unnecessary background services** | Scans for 19 known non-essential services (Xbox, Fax, telemetry, geolocation, etc.) | One-click stop for each (admin); auto-restarts on reboot |
| 22 | **Windows Search indexer** | Detects if WSearch service is running (heavy disk I/O on older PCs) | One-click stop |
| 23 | **Superfetch/SysMain** | Detects if SysMain is running (causes disk thrashing on HDDs) | One-click stop |
| 24 | **Telemetry services** | Detects DiagTrack and dmwappushservice running | One-click stop |

#### Network Issues

| # | Problem | How the App Helps | Fix Method |
|---|---|---|---|
| 25 | **Bandwidth-hogging apps** | Shows all processes with active network connections, sorted by I/O | Identifies the app; guide to close it or pause sync |
| 26 | **Slow DNS resolution** | Stale DNS cache causing slow or broken website loading | One-click DNS flush (`ipconfig /flushdns`) |
| 27 | **Background downloads** | Identifies OneDrive, Windows Update, Dropbox using bandwidth | Actionable tips to pause syncing |

#### System Health Issues

| # | Problem | How the App Helps | Fix Method |
|---|---|---|---|
| 28 | **Long uptime (no restart)** | Detects when PC hasn't restarted in 7+ days | Advises restart with explanation of why it helps |
| 29 | **Overall health score** | Calculates a 0–100 health score based on all detected issues | Prioritized fix list sorted by impact |
...
| **Running Apps** | "What's Causing Your Fan to Spin?" CPU hog section + top processes by memory with "End" buttons + memory leak detection |
...
| Fan noise / CPU hog detection | Yes | Yes |
...
| `GET` | `/api/diagnose` | Full diagnosis — issues, severity, fix instructions, impact estimate, effort, admin requirement, and name of high-CPU culprit app |
| `GET` | `/api/processes` | Top 30 processes by memory usage |
| `GET` | `/api/cpu-hogs` | All processes sorted by CPU%, with per-app fix advice and live system CPU% |
...
- **Protected system processes** — critical Windows processes (explorer.exe, svchost.exe, System Idle Process, etc.) cannot be killed or flagged as problems
...
Python isn't in your PATH. Reinstall Python from python.org and check **"Add Python to PATH"** during setup.

> Note: `py` (the Python Launcher) is installed separately from Python itself. If `py` doesn't work, use `python` instead — they do the same thing.
...
Then update `requirements.txt` to use the pywin32 version that matches your Python version (e.g. `pywin32==311` for Python 3.14).
...
3. Run the installer and tick **both**:
   - ✅ Add Python to PATH
   - ✅ Install for all users
4. Open a new Command Prompt and run `python --version` to confirm it works
...
2. Run `python app.py` (or right-click `start-as-admin.bat` → Run as administrator)
3. Wait for the message `Running on http://127.0.0.1:5555`
4. Then open `http://localhost:5555` in your browser

### Fan noise section shows "No app is using CPU" but fan is still spinning

This is **thermal lag** — your laptop's CPU may have been busy a moment ago and is now cooling down. The fan keeps spinning for 1–2 minutes after load drops, which is normal. Wait a moment and click the **Refresh** button in that section. If the fan keeps going, check **PC Health → CPU Temperature** to see if the laptop is genuinely hot.
